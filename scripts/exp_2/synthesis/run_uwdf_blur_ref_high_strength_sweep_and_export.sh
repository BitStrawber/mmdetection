...
# UWDF high-strength sweep with the new environment prompt and blurred refs.
#
# Purpose:
#   Test whether SDXL img2img + depth ControlNet + IP-Adapter can create a
#   clearly underwater global environment when the source image is denoised more
#   aggressively.
#
# All experiments use the same selected:
#   source image + Depth Anything map + heavily blurred underwater reference + text prompt
#
# Only img2img denoising strength changes:
#   e1_blurref_s050  strength=0.50
#   e2_blurref_s055  strength=0.55
#   e3_blurref_s060  strength=0.60
#   e4_blurref_s070  strength=0.70
#   e5_blurref_s075  strength=0.75
#
# Run after activating the UWDF environment:
#   conda activate /media/SSD1/conda_envs/uwdf
#   bash scripts/exp_2/synthesis/run_uwdf_blur_ref_high_strength_sweep_and_export.sh
...
WORK_ROOT="${WORK_ROOT:-/media/SSD1/XCX/exp_2/synthesis_work/uwdf_blur_ref_high_strength_sweep}"
EXP_ROOT="${EXP_ROOT:-${WORK_ROOT}/experiments}"
SELECT_ROOT="${SELECT_ROOT:-${WORK_ROOT}/selected}"
OUT_ROOT="${OUT_ROOT:-/media/HDD1/XCX/exp_2/uwdf_blur_ref_high_strength_sweep_multigrid_export}"
...
WIDTH="${WIDTH:-1024}"
STEPS="${STEPS:-20}"
STRENGTHS="${STRENGTHS:-0.50 0.55 0.60 0.70 0.75}"
GUIDANCE_SCALE="${GUIDANCE_SCALE:-8.0}"
CONTROLNET_SCALE="${CONTROLNET_SCALE:-0.85}"
IP_ADAPTER_SCALE="${IP_ADAPTER_SCALE:-0.35}"
BLUR_RADIUS="${BLUR_RADIUS:-40}"
BLUR_DOWNSAMPLE="${BLUR_DOWNSAMPLE:-16}"

PROMPT="${PROMPT:-the entire scene is submerged underwater, global blue-green water illumination, visible underwater haze across the whole image, reduced contrast over the entire scene, soft volumetric scattering, natural underwater light attenuation, same object identity, same object shape, same composition}"
...
REF_PANEL_LABEL="${REF_PANEL_LABEL:-strong blur reference}"
OVERWRITE="${OVERWRITE:-1}"

EXPERIMENTS="e1_blurref_s050 e2_blurref_s055 e3_blurref_s060 e4_blurref_s070 e5_blurref_s075"
...
echo "========================================="
echo "UWDF blur-reference high-strength sweep"
echo "========================================="
...
if [[ "${#strength_array[@]}" -ne 5 ]]; then
  echo "Error: STRENGTHS must contain exactly 5 values, got: ${STRENGTHS}" >&2
  exit 1
fi
...
echo
echo "Step 1/3: Select shared source/depth/reference samples and build strong blurred refs"
...
run_exp() {
  ...
  echo "Launch ${exp_name} on GPU ${gpu}; strength=${strength}; log=${log_file}"
  (
    cd "${UWDF_DIR}"
    GPU="${gpu}" \
...
echo
echo "Step 2/3: Run five high-strength experiments in parallel"
run_exp "e1_blurref_s050" "${gpu_array[0]}" "${strength_array[0]}"
pid1=$!
run_exp "e2_blurref_s055" "${gpu_array[1]}" "${strength_array[1]}"
pid2=$!
run_exp "e3_blurref_s060" "${gpu_array[2]}" "${strength_array[2]}"
pid3=$!
run_exp "e4_blurref_s070" "${gpu_array[3]}" "${strength_array[3]}"
pid4=$!
run_exp "e5_blurref_s075" "${gpu_array[4]}" "${strength_array[4]}"
pid5=$!

failed=0
for pair in \
  "e1_blurref_s050:${pid1}" \
  "e2_blurref_s055:${pid2}" \
  "e3_blurref_s060:${pid3}" \
  "e4_blurref_s070:${pid4}" \
  "e5_blurref_s075:${pid5}"
do
  name="${pair%%:*}"
  pid="${pair#*:}"
  if wait "${pid}"; then
