...
# UWDF strength sweep with the new environment prompt and blurred references.
#
# All experiments use the same selected:
#   source image + Depth Anything map + blurred underwater reference + text prompt
#
# Only img2img denoising strength changes:
#   e1_blurref_s020  strength=0.20
#   e2_blurref_s025  strength=0.25
#   e3_blurref_s030  strength=0.30
#   e4_blurref_s035  strength=0.35
#   e5_blurref_s040  strength=0.40
#
# Run after activating the UWDF environment:
#   conda activate /media/SSD1/conda_envs/uwdf
#   bash scripts/exp_2/synthesis/run_uwdf_blur_ref_strength_sweep_and_export.sh
...
WORK_ROOT="${WORK_ROOT:-/media/SSD1/XCX/exp_2/synthesis_work/uwdf_blur_ref_strength_sweep}"
EXP_ROOT="${EXP_ROOT:-${WORK_ROOT}/experiments}"
SELECT_ROOT="${SELECT_ROOT:-${WORK_ROOT}/selected}"
OUT_ROOT="${OUT_ROOT:-/media/HDD1/XCX/exp_2/uwdf_blur_ref_strength_sweep_multigrid_export}"
...
WIDTH="${WIDTH:-1024}"
STEPS="${STEPS:-20}"
STRENGTHS="${STRENGTHS:-0.20 0.25 0.30 0.35 0.40}"
...
echo "========================================="
echo "UWDF blur-reference strength sweep"
echo "========================================="
...
echo
echo "Step 2/3: Run five strength experiments in parallel"
run_exp "e1_blurref_s020" "${gpu_array[0]}" "${strength_array[0]}"
pid1=$!
run_exp "e2_blurref_s025" "${gpu_array[1]}" "${strength_array[1]}"
pid2=$!
run_exp "e3_blurref_s030" "${gpu_array[2]}" "${strength_array[2]}"
pid3=$!
run_exp "e4_blurref_s035" "${gpu_array[3]}" "${strength_array[3]}"
pid4=$!
run_exp "e5_blurref_s040" "${gpu_array[4]}" "${strength_array[4]}"
pid5=$!

failed=0
for pair in \
  "e1_blurref_s020:${pid1}" \
  "e2_blurref_s025:${pid2}" \
  "e3_blurref_s030:${pid3}" \
  "e4_blurref_s035:${pid4}" \
  "e5_blurref_s040:${pid5}"
do
  name="${pair%%:*}"
  pid="${pair#*:}"
  if wait "${pid}"; then
