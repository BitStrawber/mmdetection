...
# Five-way UWDF ablation for the new environment prompt.
#
# The five experiments use the same selected ImageNet source images, Depth
# Anything maps, underwater references, prompt, negative prompt, ControlNet
# scale, and IP-Adapter scale. Only img2img strength and text guidance differ:
#
#   e1_prompt_s040_gbase       strength=0.40, guidance=5.0
#   e2_prompt_s050_gbase       strength=0.50, guidance=5.0
#   e3_prompt_sbase_gstrong    strength=0.30, guidance=10.0
#   e4_prompt_s040_gstrong     strength=0.40, guidance=10.0
#   e5_prompt_s050_gstrong     strength=0.50, guidance=10.0
#
# Run after activating the UWDF environment:
#   conda activate /media/SSD1/conda_envs/uwdf
#   bash scripts/exp_2/synthesis/run_uwdf_prompt_strength_textweight_five_ablation_and_export.sh
...
WORK_ROOT="${WORK_ROOT:-/media/SSD1/XCX/exp_2/synthesis_work/uwdf_prompt_strength_textweight_five_ablation}"
EXP_ROOT="${EXP_ROOT:-${WORK_ROOT}/experiments}"
SELECT_ROOT="${SELECT_ROOT:-${WORK_ROOT}/selected}"
OUT_ROOT="${OUT_ROOT:-/media/HDD1/XCX/exp_2/uwdf_prompt_strength_textweight_five_ablation_multigrid_export}"
...
BASE_STRENGTH="${BASE_STRENGTH:-0.30}"
STRENGTH_040="${STRENGTH_040:-0.40}"
STRENGTH_050="${STRENGTH_050:-0.50}"
BASE_GUIDANCE_SCALE="${BASE_GUIDANCE_SCALE:-5.0}"
HIGH_GUIDANCE_SCALE="${HIGH_GUIDANCE_SCALE:-10.0}"
...
EXPERIMENTS="e1_prompt_s040_gbase e2_prompt_s050_gbase e3_prompt_sbase_gstrong e4_prompt_s040_gstrong e5_prompt_s050_gstrong"
...
echo "========================================="
echo "UWDF prompt strength/text-weight ablation"
echo "========================================="
...
echo "BASE_STRENGTH:         ${BASE_STRENGTH}"
echo "STRENGTH_040:          ${STRENGTH_040}"
echo "STRENGTH_050:          ${STRENGTH_050}"
...
for path in [source_out, depth_out, ref_out]:
    clear_tree(path)
...
if len(classes) >= num:
    ...
    picked_pairs = [rng.choice(sorted(by_cls[cls], key=lambda x: str(x[0]))) for cls in picked_classes]
else:
    all_pairs = sorted((x for xs in by_cls.values() for x in xs), key=lambda x: str(x[0]))
    picked_pairs = rng.sample(all_pairs, min(num, len(all_pairs)))
...
picked_refs = rng.sample(refs_all, len(picked_pairs)) if len(refs_all) >= len(picked_pairs) else [rng.choice(refs_all) for _ in picked_pairs]

records = []
for idx, ((source, depth, rel), ref) in enumerate(zip(picked_pairs, picked_refs)):
    source_dst = source_out / rel
    depth_dst = (depth_out / rel).with_suffix(".png")
    ref_dst = ref_out / f"{idx:08d}{ref.suffix.lower()}"
    source_dst.parent.mkdir(parents=True, exist_ok=True)
    depth_dst.parent.mkdir(parents=True, exist_ok=True)

    for dst in [source_dst, depth_dst, ref_dst]:
...
run_exp() {
  ...
  echo "Launch ${exp_name} on GPU ${gpu}; strength=${strength}; guidance=${guidance_scale}; log=${log_file}"
  (
    cd "${UWDF_DIR}"
    GPU="${gpu}" \
...
run_exp "e1_prompt_s040_gbase" "${gpu_array[0]}" "${STRENGTH_040}" "${BASE_GUIDANCE_SCALE}"
pid1=$!
run_exp "e2_prompt_s050_gbase" "${gpu_array[1]}" "${STRENGTH_050}" "${BASE_GUIDANCE_SCALE}"
pid2=$!
run_exp "e3_prompt_sbase_gstrong" "${gpu_array[2]}" "${BASE_STRENGTH}" "${HIGH_GUIDANCE_SCALE}"
pid3=$!
run_exp "e4_prompt_s040_gstrong" "${gpu_array[3]}" "${STRENGTH_040}" "${HIGH_GUIDANCE_SCALE}"
pid4=$!
run_exp "e5_prompt_s050_gstrong" "${gpu_array[4]}" "${STRENGTH_050}" "${HIGH_GUIDANCE_SCALE}"
pid5=$!

failed=0
for pair in \
  "e1_prompt_s040_gbase:${pid1}" \
  "e2_prompt_s050_gbase:${pid2}" \
  "e3_prompt_sbase_gstrong:${pid3}" \
  "e4_prompt_s040_gstrong:${pid4}" \
  "e5_prompt_s050_gstrong:${pid5}"
do
  name="${pair%%:*}"
  pid="${pair#*:}"
  if wait "${pid}"; then
