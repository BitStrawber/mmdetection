#!/usr/bin/env bash
# Train the four ImageNet-1K -> DFUI detector-backbone variants, then
...
# Keep high-churn training artifacts off the nearly full system volume and
# dataset HDD.  Callers can still override every output location.
OUTPUT_ROOT="${OUTPUT_ROOT:-/media/SSD1/XCX/exp_2/dfui_training_runs/imagenet1k_100e}"
...
IMAGENET1K_R50_RAW="${IMAGENET1K_R50_RAW:-$REPO_ROOT/work_dirs/tri_pretrain/imagenet_dino_resnet50_100e/checkpoint.pth}"
IMAGENET1K_VITS_RAW="${IMAGENET1K_VITS_RAW:-$REPO_ROOT/work_dirs/tri_pretrain/imagenet_dino_vits_100e/checkpoint.pth}"
IMAGENET1K_R50_INIT="$PRETRAIN_DIR/${PRETRAIN_PREFIX}_dino_resnet50_100e_teacher_backbone.pth"
IMAGENET1K_VITS_INIT="$PRETRAIN_DIR/${PRETRAIN_PREFIX}_dino_vits_100e_teacher_backbone.pth"
...
run_architecture_pipeline() {
    ...
    case "$architecture" in
      resnet50)
        raw="$IMAGENET1K_R50_RAW"
        init="$IMAGENET1K_R50_INIT"
        ruod_config="$R50_RUOD_CONFIG"
        expected_arch=resnet50
        prepend=""
        ;;
      vits)
        raw="$IMAGENET1K_VITS_RAW"
        init="$IMAGENET1K_VITS_INIT"
    ...
    for variant in "${VARIANT_LIST[@]}"; do
      ...
      if [ "$variant" = "dfui_ruod" ]; then
        ...
        if [ "$architecture" = "resnet50" ]; then
          if [ -n "$R50_DFUI_RUOD_CONFIG" ]; then
            dfui_config="$R50_DFUI_RUOD_CONFIG"
          else
        ...
        if [ "$architecture" = "resnet50" ]; then
          if [ -n "$R50_DFUI_RUOD_UIIS_CONFIG" ]; then
            dfui_config="$R50_DFUI_RUOD_UIIS_CONFIG"
          else
...
if [ "$CHECK_ONLY" = "1" ]; then
  if [ -n "$R50_DFUI_RUOD_CONFIG" ]; then
    echo "R50 DFUI+RUODEasy config: $R50_DFUI_RUOD_CONFIG"
  else
    echo "R50 DFUI+RUODEasy config: $(resolve_unique_config "$R50_DFUI_RUOD_J10_DIR" "R50 DFUI+RUODEasy J10 configuration")"
  fi
  if [ -n "$R50_DFUI_RUOD_UIIS_CONFIG" ]; then
    echo "R50 DFUI+RUODEasy+UIISEasy config: $R50_DFUI_RUOD_UIIS_CONFIG"
  else
